...
rem ---- 还原备份（如指定）----
rem   tarball 由 migrate-to-windows.cmd 生成，里面可能有：
rem     .env                        配置
rem     server\data\ENCRYPTION_KEY  AES-GCM 密钥
rem     server\data\state\          D1 数据库（含加密的主机记录 + 密码 + 私钥）
rem   覆盖到 !INSTALL_DIR! 时路径自动对齐。
...
if exist "!INSTALL_DIR!\server\data\ENCRYPTION_KEY" (
    for /f "usebackq delims=" %%k in (`tar -tzf "!BACKUP!" 2^>nul ^| findstr /x "server\data\ENCRYPTION_KEY"`) do (
        if not "%%k" == "" (
            ...
            echo [deploy] 已备份远端旧 ENCRYPTION_KEY -^> ENCRYPTION_KEY.old-!STAMP!（防止新 key 覆盖后丢数据）
        )
    )
)
